...
async function cadastrar(req, res) {
  ...
  const info = db.prepare('INSERT INTO usuarios (nome, email, senha_hash) VALUES (?, ?, ?)').run(nomeLimpo, email, hash);
  const usuario = { id: info.lastInsertRowid, nome: nomeLimpo, email };
...
async function loginGoogle(req, res) {
  ...
  const { sub: googleId, name: nome } = payload;
  const email = normalizarEmail(payload.email);
  if (!payload.email_verified || !email || !googleId) {
    return res.status(401).json({ erro: 'A conta Google precisa ter um e-mail verificado.' });
  }
